...
export default function Home() {
    ...
    try {
      ...
      const challenge =
        dailyChallenge &&
          ...
          ? dailyChallenge
          : await fetchDailyChallenge(locale);

      setDailyChallenge(challenge);
      setIsDailyMode(true);
      setGoal(challenge.goal.title);
      const ref = await countReferer(challenge.goal.title, locale);
      setNumOfReferer(ref.numOfRef);
      setHints(ref.hints);

      const url = `https://${locale}.wikipedia.org/w/api.php?action=parse&pageid=${challenge.goal.id}&format=json&origin=*`;
  ...
  const fetchTitle = async (title: string) => {
    ...
    try {
      ...
      if (ignoreNextContentRef.current) {
        ignored = true;
  ...
  const handleBackClick = () => {
    ...
    setHistory((prev) => prev.slice(0, Math.max(prev.length - 1, 0)));
    setStroke((prev) => Math.max(prev - 1, -1));
  };
  ...
  const start = async (mode: StartMode = "random") => {
    ...
    if (mode === "daily") {
      ...
      setContent("");
      const challenge = await prepareDailyGoal();
      ignoreNextContentRef.current = false;
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-6 py-4">
          <div>
            <p className="text-ml uppercase tracking-[0.3em] text-slate-400">打数:<span className="text-4xl font-semibold text-white">
              {stroke === -1 ? "0" : stroke}
            </span></p>
      ...
      <main className="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-8 lg:flex-row">
        <aside className="flex w-full flex-col gap-6 lg:w-80 lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
          <section className="rounded-3xl bg-gradient-to-br from-blue-500 via-indigo-500 to-slate-900 p-6 text-white shadow-2xl">
            <p className="text-xs uppercase tracking-[0.3em] text-white/70">今日のお題</p>
          ...
          {goal && (
            <section className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-xl backdrop-blur">
              ...
              >
                <div>
                  <p className="text-xs uppercase tracking-wide text-slate-300">ゴール記事</p>
          ...
          <section className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white shadow-xl backdrop-blur">
            <div className="flex items-center justify-between">
              ...
              <button
                className={`text-xs transition ${history.length === 0
            ...
            <div className="mt-4 max-h-[50vh] space-y-3 overflow-y-auto pr-2">
              {history.length === 0 ? (
                <p className="text-sm text-slate-300">まだ遷移履歴がありません。</p>
              ) : (
